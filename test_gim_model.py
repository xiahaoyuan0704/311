...
from gim_model import flatten_layers, load_gim, save_gim
...
class GimModelTests(unittest.TestCase):
    def test_load_and_flatten(self) -> None:
        ...
        flat = flatten_layers(doc.layers)
        ids = [layer.id for layer in flat]
        self.assertEqual(ids, ["bg-01", "group-01", "text-01"])

    def test_round_trip_save(self) -> None:
        doc = load_gim("sample.gim")
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "out.gim"
            save_gim(path, doc)
            loaded = load_gim(path)
